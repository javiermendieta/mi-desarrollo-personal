...
export async function saveSocialMediaPostToDB(post: {
  id?: string;
  platform?: string;
  content?: string;
  hashtags?: string[];
  ...
  if (!res.ok) {
    ...
    throw new Error('Error saving social media post');
  }
  return res.json();
}

export async function deleteSocialMediaPostFromDB(id: string) {
  const res = await fetch(`/api/social-media-posts?id=${id}`, { method: 'DELETE' });
  if (!res.ok) throw new Error('Error deleting social media post');
  return res.json();
}

// Medical Appointments API
export async function saveMedicalAppointmentToDB(appointment: {
  id?: string;
  title: string;
  doctor?: string;
  specialty?: string;
  ...
  // Siempre usar POST con upsert (crear o actualizar)
...
export async function deleteMedicalAppointmentFromDB(id: string) {
  const res = await fetch(`/api/medical-appointments?id=${id}`, { method: 'DELETE' });
  if (!res.ok) throw new Error('Error deleting medical appointment');
  return res.json();
}

// Medical Tasks API
export async function saveMedicalTaskToDB(task: {
  id?: string;
  title: string;
  category?: string;
  dueDate?: string;
  ...
  // Siempre usar POST con upsert (crear o actualizar)
  const res = await fetch('/api/medical-tasks', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  if (!res.ok) {
    const error = await res.text();
    console.error('Error saving medical task:', error);
    throw new Error('Error saving medical task');
  }
  return res.json();
}

export async function deleteMedicalTaskFromDB(id: string) {
  const res = await fetch(`/api/medical-tasks?id=${id}`, { method: 'DELETE' });
  if (!res.ok) throw new Error('Error deleting medical task');
  return res.json();
}
