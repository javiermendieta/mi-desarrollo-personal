...
import { saveConversationToDB, deleteConversationFromDB } from '@/lib/dbApi';
...
export function AIAssistant({ isOpen, onClose }: AIAssistantProps) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const isFirstMessage = (updatedConv?.messages?.length || 0) === 0;
      const title = isFirstMessage ? message.slice(0, 30) + (message.length > 30 ? '...' : '') : updatedConv?.title;

      // Sync to DB - guardar conversación completa
      ...
      try {
        ...
        console.error('Error syncing conversation to DB:', e);
      }
    } catch (error) {
  ...
  const handleDeleteConversation = async (convId: string) => {
    ...
    try {
      ...
      console.error('Error deleting conversation from DB:', e);
    }
  };
