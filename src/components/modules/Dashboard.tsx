...
export function Dashboard({ onNavigate }: DashboardProps) {
  ...
  // Today's events - extract date directly from ISO string
  const todayEvents = events.filter((e) => {
    const eventDate = e.startDate.replace('Z', '').slice(0, 10);
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {(() => {
                          // Extract time directly from ISO string to preserve exact time
                          const timePart = event.startDate.replace('Z', '').slice(11, 16);
                          return timePart;
                        })()}
                      </p>
                    </div>
